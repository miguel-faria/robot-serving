...
namespace movement_decision {
	...
	bool surrounded(Point3f cup, map<string, Point3f> pos_map, string current_id){
		...
		if(euclidean_distance(cup, cup1) < 250 && euclidean_distance(cup, cup2) < 250)
			return true;
		else
			return false;
	}
	...
	bool side_close(Point3f cup, map<string, Point3f> pos_map, string arm, string current_id){
		...
		if(arm.compare("left") == 0 || arm.compare("Left") == 0){
			if(cup1.x > cup.x && (euclidean_distance(cup, cup1) <= 250 || euclidean_distance(cup, cup2) <= 250))
				return true;
		} else {
			if(cup1.x < cup.x && (euclidean_distance(cup, cup1) <= 250 || euclidean_distance(cup, cup2) <= 250))
				return true;
	...
	bool outmost_cup(Point3f cup, map<string, Point3f> pos_map, string arm){
		...
		for(map<string, Point3f>::iterator it = pos_map.begin(); it != pos_map.end(); it++)
			if((left_arm && it->second.x > cup.x) ||
					(!left_arm && it->second.x < cup.x))
				return false;

		return true;
	}

	bool equally_spaced_cups(double current_cup_dist, map<string, double> dists){

		for(map<string, double>::iterator it = dists.begin(); it != dists.end(); it++)
			if(it->second != current_cup_dist)
				return false;
	...
	ros::ServiceClient Mixed_Trajectory::choose_trajectory_service(map<string, Point3f> cups,
		...
		else if(outmost_cup(current_cup_pos, pos, arm) || (!surround && !laterally_next) ||
				equally_spaced_cups(current_cup_dist, dists))
