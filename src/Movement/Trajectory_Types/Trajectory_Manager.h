...
namespace movement_decision {
	...
	class Trajectory_Manager {
		...
		public:
			...
			string get_trajectory( ) const {
				return _trajectory;
			}

			ros::ServiceClient get_feedback_traj_serv( ) const {
				return _feedback_traj_serv;
			}

			ros::ServiceClient get_restart_traj_serv( ) const {
				return _restart_traj_serv;
			}

			ros::ServiceClient get_send_traj_serv( ) const {
				return _send_traj_serv;
			}

			void set_trajectory(string trajectory) {
				_trajectory = trajectory;
			}

			void set_feedback_traj_serv(ros::ServiceClient feedback_traj_serv) {
				_feedback_traj_serv = feedback_traj_serv;
			}

			void set_restart_traj_serv(ros::ServiceClient restart_traj_serv) {
				_restart_traj_serv = restart_traj_serv;
			}

			void set_send_traj_serv(ros::ServiceClient send_traj_serv) {
				_send_traj_serv = send_traj_serv;
			}

			ros::ServiceClient choose_trajectory_service(map<string, Point3f>, map<string, double> cups_dist,
					string current_cup_id, string arm) { return ros::ServiceClient(); }
