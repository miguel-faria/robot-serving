...
namespace movement_decision {
	...
	class Mixed_Trajectory : public Trajectory_Manager {
		...
		public:
			...
			ros::ServiceClient get_legible_trajectory_srv( ) const {
				return _legible_trajectory_srv;
			}

			ros::ServiceClient get_predictable_trajectory_srv( ) const {
				return _predictable_trajectory_srv;
			}

			void set_legible_trajectory_srv(ros::ServiceClient legible_trajectory_srv) {
				_legible_trajectory_srv = legible_trajectory_srv;
			}

			void set_predictable_trajectory_srv(ros::ServiceClient predictable_trajectory_srv) {
				_predictable_trajectory_srv = predictable_trajectory_srv;
			}

			virtual ros::ServiceClient choose_trajectory_service(map<string, Point3f> cups,
